...
- Polymaker PolyLite ASA ([AMAZON_LINK]) — one of the most popular and consistent ASA options
- Bambu Lab ASA ([BAMBU_LINK]) — dialled in for Bambu enclosed printers, minimal tuning required
- eSUN ASA ([AMAZON_LINK]) — good budget option
...
**Recommended:** Bambu Lab PA6-CF ([BAMBU_LINK]) for structural parts where stiffness matters.
...
Requires a hardened steel nozzle — the carbon fibre will wreck a standard brass nozzle quickly. Check the current price for hardened nozzles on Amazon ([AMAZON_LINK]).
...
- **Consider post-processing:** UV-resistant spray paint or clear coat can significantly extend the life of PETG outdoor parts. Rustoleum and Krylon both make UV-blocking clear coats ([AMAZON_LINK])
...
1. **Get an enclosed printer** if you don't have one — ASA really does need it. The **Bambu Lab P1S** ([BAMBU_LINK]) is an excellent enclosed option with pre-tuned ASA profiles.
2. **Buy Polymaker PolyLite ASA** ([AMAZON_LINK]) as your go-to outdoor filament
3. **Use PETG** ([AMAZON_LINK]) for less critical outdoor parts or if you only have an open-frame printer
