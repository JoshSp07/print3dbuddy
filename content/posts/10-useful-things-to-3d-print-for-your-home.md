...
**Bearings needed:** most designs use 608 bearings (the same ones in fidget spinners) — you can grab a pack cheaply on Amazon ([AMAZON_LINK])
...
If you don't have a printer yet, the **Bambu Lab A1 Mini** ([BAMBU_LINK]) is an outstanding beginner machine that produces excellent results for all the prints listed here. It's fast, accurate, and requires almost no setup. Check the current price on the Bambu Lab website ([BAMBU_LINK]).

On a tighter budget? The **Creality Ender 3 V3 SE** ([AMAZON_LINK]) is one of the best entry-level printers available — check the current price on Amazon ([AMAZON_LINK]).

And pick up a couple of spools of PLA to start — Hatchbox ([AMAZON_LINK]) and eSUN PLA+ ([AMAZON_LINK]) are reliable, affordable choices that print beautifully for home organisation projects.
